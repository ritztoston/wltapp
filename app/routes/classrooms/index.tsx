...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const classrooms = await getClassrooms(user.id, cursor);
...
export default function ClassroomsPage() {
  ...
  useEffect(() => {
    if (fetcher.data && fetcher.data.classrooms.length === 0) {
      return setShouldFetch(false);
    }

    if (fetcher.data && fetcher.data.classrooms.length > 0) {
      const newItems = fetcher.data.classrooms;
      setClassrooms((x) => [...x, ...newItems]);
      setShouldFetch(true);
    }
  }, [fetcher.data]);
          ...
          {classrooms.length ? (
            <>
              ...
              {classrooms.map((classroom, index) => (
                ...
                >
                  <div
                    ref={index === classrooms.length - 1 ? ref : undefined}
              ...
              {isLoading ? (
                <ArrowDownCircleIcon className="col-span-1 sm:col-span-2 lg:col-span-4 justify-self-center animate-bounce h-10 w-10 text-main-blue" />
              ) : null}
            </>
          ) : (
            <button
